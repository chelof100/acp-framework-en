...
% ACP v1.25 — Admission Control for Agent Actions
...
\hypersetup{
  ...
  pdfsubject={Formal protocol for governance of autonomous agents. v1.25. arXiv:2603.18829},
...
\title{%
  ...
  \large ACP v1.25: Admission Control for Agent Actions\\[0.3em]
...
\date{April 2026 \\ \small Draft Standard \\ \small \href{https://arxiv.org/abs/2603.18829}{arXiv:2603.18829 [cs.CR]} \quad \href{https://doi.org/10.5281/zenodo.19449650}{DOI: 10.5281/zenodo.19449650}}
...
(11~invariants + 4~temporal properties, 0~violations across 5,684,342~states)
and validated at runtime by 73~signed conformance test vectors.
We further identify \emph{deviation collapse}: a degenerate regime in which enforcement is
active but never exercised because upstream constraints eliminate the conditions required
for \texttt{DENIED} decisions.
We introduce Boundary Activation Rate (BAR) as a metric and counterfactual evaluation
as a detection mechanism (Experiment~9), and validate $\Delta\mathrm{BAR}$ early-warning
detection under progressive drift (Phase~D).
...
A useful analogy for understanding what ACP does is the Kubernetes Admission Controller pattern.
...
This separation enables a protocol that is both efficient in isolation (767--921\,ns per
decision, p50; Table~\ref{tab:benchmarks}, $\sim$88\,ns for the cooldown short-circuit path) and adaptable in deployment:
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Formal verification of safety invariants.} Safety properties
    ...
    (11~invariants + 4~temporal properties, 0~violations), establishing that
  ...
  \item \textbf{Knowledge-aware adversarial evasion (Experiment~10).}
    ...
    The three v1.25 mechanisms form a coherent defense against knowledge-aware evasion.
\end{itemize}
...
ACP and AgentSpec are complementary: where AgentSpec focuses on general-purpose
safety rules expressed in a user-defined DSL, ACP targets institutional admission
control with cryptographic identity, formal policy verification, and an append-only
audit ledger.
...
The key derivation adds a single SHA-256 call per evaluation (767\,ns;
...
ACP v1.25 is a complete Draft Standard specification with a full Go reference implementation
and external verifiability through a TLC-checked formal model and an ACR-1.0 sequence compliance runner.

\subsection{Active Specifications --- v1.25 (38 documents)}
...
\bottomrule
\end{tabular}
\caption{ACP v1.25 active specification documents by conformance level (38 primary documents shown).
...
The \texttt{pkg/risk} engine (ACP-RISK-2.0/3.0) was benchmarked using \texttt{go test -bench} on an Intel Core i7-8665U @ 1.90GHz (Go 1.22, Windows, \texttt{GOMAXPROCS=8}).
...
\bottomrule
\end{tabular}
\caption{ACP-RISK-2.0/3.0 engine benchmarks (\texttt{go test -bench=. -benchtime=3s},
...
ACP-RISK-3.0 preserves these latency bounds; context-scoped key derivation adds a single \texttt{SHA-256} call per evaluation (767\,ns; Table~\ref{tab:benchmarks}, \texttt{PatternKey} row), a cost dominated by existing ledger access in production deployments.
...
All experiments use the Go reference implementation (v1.25) on an Intel Core i7-8665U @ 1.90GHz (Go~1.22~\cite{go-language}, \texttt{GOMAXPROCS=8}).
...
cheap (767--921\,ns, p50; Table~\ref{tab:benchmarks}) in all three configurations.
...
the \texttt{Evaluate()} decision function contributes 767--921\,ns per request
...
The \texttt{Evaluate()} function contributes a fixed 767--921\,ns per request (p50; Table~\ref{tab:benchmarks})
...
\emph{Scope.}
ACP v1.25 enforces guarantees at the per-agent level. Cross-agent coordination is
...
Decision evaluation incurs minimal overhead (767--921\,ns, p50; Table~\ref{tab:benchmarks}), while system throughput is bounded
...
\bottomrule
\end{tabular}
\caption{ACP v1.25 development roadmap.}
\label{tab:roadmap}
\end{table}
...
mechanism.
As a consequence, the 9.5$\times$ short-circuit path measured in
...
meaning the cooldown mechanism is never activated and the $9.5\times$ short-circuit
path (88\,ns, Section~\ref{sec:adversarial}) is never exercised.
...
Together, the three v1.25 mechanisms form a coherent response to this attack class.
...
a TLC-runnable TLA+ formal model (base: 3~invariants; extended v1.25: 11~invariants + 4~temporal properties, 5,684,342~states generated, 3,147,864~distinct, graph depth~15, 0~violations within the bounded state space),
...
\textbf{Preprint:} \url{https://arxiv.org/abs/2603.18829} \quad \textbf{Zenodo:} \href{https://doi.org/10.5281/zenodo.19449650}{10.5281/zenodo.19449650}\\
...
\texttt{LedgerQuerier} abstraction keeps the evaluation function at 767--921\,ns (p50; Table~\ref{tab:benchmarks}) while
