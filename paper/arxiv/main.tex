...
decisions that existing stateless frameworks (RBAC, ABAC, OPA, Cedar) are not designed to enforce.
...
security properties, however, are conditioned on the integrity of the
deployed backend (see \S\ref{sec:security-model}).
...
Step~2 early exit), and throughput degrading predictably to $\sim$712k~req/s at 500 workers
due to state backend latency, as isolated in Experiment~3.
...
that ACP-RISK-2.0 enforcement resists the evaluated evasion scenarios: 99\% (495/500) of
single-agent evasion attempts are blocked after only five requests (Experiment~1),
per-agent isolation is preserved across 100 coordinated agents (Experiment~2),
throughput degradation under stress is attributable solely to state-backend latency
rather than decision logic (Experiment~3), and repeated identical tokens trigger
F\_anom Rule~3 escalation from ESCALATED to DENIED after three occurrences
in a 5-minute window (Experiment~4).
ACP provides end-to-end verifiability through 73~signed conformance test vectors,
a TLC-checked TLA+~formal model (base: 3~invariants; extended v1.20: 9~invariants
+ 4~temporal properties, 5,684,342~states generated, 3,147,864~distinct, graph depth~15, 0~violations within the bounded state space), and an ACR-1.0 sequence
...
Stateless policy engines evaluate requests in isolation and can enforce safety properties
over individual interactions; they do not capture temporal behavioral properties that
require reasoning over accumulated execution history.
ACP addresses this gap by enforcing admission decisions over stateful interaction traces.
...
\subsection{Active Specifications --- v1.20 (38 documents)}
...
\bottomrule
\end{tabular}
\caption{ACP v1.20 active specification documents by conformance level (38 primary documents shown).
...
All experiments use the Go reference implementation (v1.20) on an Intel Core i7-8665U @ 1.90GHz (Go~1.22~\cite{go-language}, \texttt{GOMAXPROCS=8}).
...
\begin{tabular}{lrrrl}
\toprule
Backend                    & RTTs/req & Throughput (req/s) & Duration       & Relative \\
\midrule
...
Relative column: InMemoryQuerier and RedisQuerier are normalized to InMemoryQuerier (1.0$\times$ baseline);
RedisPipelinedQuerier (1.7$\times$) is normalized to unpipelined Redis, showing the pipelining gain.}
\label{tab:adv-exp3}
\end{table}
...
\textbf{State-Mixing and Context Fragmentation.}
...
This is identified as a targeted extension to ACP-RISK-2.0 for future specification versions.
...
scope of the bounded model (planned for v1.21).
