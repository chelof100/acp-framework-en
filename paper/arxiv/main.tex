...
New in v1.20: token replay adversarial evaluation (Experiment~4, F\_anom Rule~3 escalation, bounded replay resistance); Deployment Considerations section (\S\ref{sec:deployment}); Redis pipelining (\texttt{RedisPipelinedQuerier}, 2~RTTs/request, $\sim$1.7$\times$ vs unpipelined Redis, Experiment~3 updated with 3 backends).
...
\midrule
InMemoryQuerier            & $\sim$1  & $\sim$15\,ms      & $\sim$600{,}000\,req/s \\
RedisQuerier (unpipelined) & $\sim$7--8 & $\sim$2.0\,s   & $\sim$4{,}700\,req/s \\
RedisPipelinedQuerier      & 2        & $\sim$1.2\,s      & $\sim$8{,}000\,req/s \\
\bottomrule
\end{tabular}
...
Pipelining reduces per-request round-trips from $\sim$7--8 to 2, yielding
$\sim$1.7$\times$ throughput gain over single-command Redis.}
\label{tab:adv-exp3}
\end{table}
...
The $\sim$1.7$\times$ factor is a conservative lower bound for this workload:
once an agent enters cooldown, \texttt{Evaluate} short-circuits at Step~2
(\texttt{CooldownActive} is true), so the unpipelined backend also performs
fewer reads per cooldown request.
For workloads without cooldown saturation, the full 4-read pipeline advantage
applies and the speedup approaches the theoretical $\sim$3--4$\times$.
The key architectural result is unchanged: ACP protocol semantics are stable
across backends; deployment performance scales with the state backend,
not the admission control logic.
