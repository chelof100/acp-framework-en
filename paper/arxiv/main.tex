...
% ACP v1.22 — Admission Control for Agent Actions
...
\hypersetup{
  ...
  pdfsubject={Formal protocol for governance of autonomous agents. v1.22. arXiv:2603.18829},
...
\title{%
  ...
  \large ACP v1.22: Admission Control for Agent Actions\\[0.3em]
...
denial at the same threshold (Experiment~7).
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Context-scoped anomaly model eliminating cross-context interference.}
    ...
    (Experiment~7, Section~\ref{sec:exp7}).
\end{itemize}
...
ACP v1.22 is a complete Draft Standard specification with a full Go reference implementation
and external verifiability through a TLC-checked formal model and an ACR-1.0 sequence compliance runner.

\subsection{Active Specifications --- v1.22 (38 documents)}
...
\bottomrule
\end{tabular}
\caption{ACP v1.22 active specification documents by conformance level (38 primary documents shown).
...
New in v1.22: ACP-RISK-3.0 implemented --- Rule~1 redefined to use context-scoped \texttt{CountPattern(ctxKey, 60s)} instead of \texttt{CountRequests(agentID, 60s)}, eliminating cross-context state-mixing (Experiment~6 fix); Experiment~7 (\texttt{TestStateMixingFixed}, 3 scenarios: clean RS=50 ESCALATED, state-mixing attack RS=50 ESCALATED, same-context burst RS=85 DENIED); ACP-RISK-3.0 $F_{\text{anom}}$ formal definition (\S\ref{sec:limitations}); Bounded Distributed Evasion limitation; new Contributions bullet; \texttt{engine\_v2\_test.go} updated for context-scoped Rule~1.
...
All experiments use the Go reference implementation (v1.22) on an Intel Core i7-8665U @ 1.90GHz (Go~1.22~\cite{go-language}, \texttt{GOMAXPROCS=8}).
...
\subsubsection*{Experiment 7: Context-Scoped Anomaly Fix (ACP-RISK-3.0)}
\label{sec:exp7}
...
\textbf{Setup.}
We reproduce the state-mixing scenario from Experiment~6 under identical
...
\begin{table}[h]
\centering
\caption{Experiment~7 --- Context-Scoped Anomaly Enforcement}
\label{tab:exp7}
\begin{tabular}{lccc}
\toprule
...
\textbf{State-Mixing Vulnerability (ACP v1.21--v1.22).}
...
This fix is implemented in ACP-RISK-3.0 and empirically validated in Experiment~7
(Section~\ref{sec:exp7}).
...
\emph{Scope.}
ACP v1.22 enforces guarantees at the per-agent level. Cross-agent coordination is
...
Experiment~5: Stateless vs.\ ACP (0.4\% vs.\ 100\% autonomous execution rate) & v1.21 Complete \\
Experiment~6: State-Mixing Vulnerability (cross-context Rule~1 contamination) & v1.21 Complete \\
ACP-RISK-3.0: context-scoped Rule~1 (\texttt{CountPattern} replaces \texttt{CountRequests}) & v1.22 Complete \\
Experiment~7: Context-Scoped Anomaly Fix (state-mixing eliminated, enforcement preserved) & v1.22 Complete \\
...
\bottomrule
\end{tabular}
\caption{ACP v1.22 development roadmap.}
\end{table}
...
ACP~v1.22 enables end-to-end verifiability by aligning formal specification,
...
This is the connection across the three layers of the v1.22 verifiability stack:
...
The v1.22 specification is complete.
...
adversarial evaluation across four attack scenarios including token replay (Experiment~4),
a stateless vs.\ stateful comparison experiment (Experiment~5),
a state-mixing vulnerability characterization (Experiment~6),
and a context-scoped anomaly fix under ACP-RISK-3.0 (Experiment~7)
